...
flags.DEFINE_string('learning_algo', 'aysnadam', 'adam or sgd')
...
def save_emb(task_id, worker_count, global_step, master, graph_input, params):
    ...
    i_writer = None
    if FLAGS.user_emb:
        u_writer = tf.python_io.TableWriter(FLAGS.outputs, slice_id=FLAGS.task_index)
    else:
        i_writer = tf.python_io.TableWriter(FLAGS.outputs, slice_id=FLAGS.task_index)
